...
use crate::modbus::protocol::CompareOp;
...
impl ScriptEngine {
    ...
    fn check_stable(&self, register: &str, seconds: u64, registers: &HashMap<String, u16>) -> bool {
        let Some(current) = registers.get(register) else { return false; };
        let last = self.last_values.get(register);
        
        if last == Some(current) {
            // Value unchanged - would need timer tracking for full implementation
            // Simplified: just check if value is stable
            true
        } else {
            false
        }
